...
let txtTerm = document.querySelector("input");
let ddlClass = document.querySelector('select');
let cbTag = document.querySelectorAll(".tagList");
let checkedTagList = [];


function printParagraph(item) {
  let p = document.createElement("p");
  p.innerHTML = `Term : ${item.term}<br/>Class : ${item.class}<br/>Definition : ${item.definition}<br/>Tag : ${item.tags}`;
  div.appendChild(p);
}

function loadAll() {
  let p = document.createElement("p");
  p.innerHTML = `<b>Total records: ${glossary.length}</b>`;
  div.appendChild(p);
...
function searchCheckbox(checkedTag) {
  div.innerHTML = '';
  checkedTag.forEach((cb) => {
    glossary
    .filter((item) => item.tags.includes(cb))
    .forEach((item) => {
      printParagraph(item);
    })
  });
...
function searchAll(inputtedTerm, selectedClass, checkedTag) {
  let result = [];
  let filterArray = [];
  let crossCheck = [];

  div.innerHTML = '';
  filterArray = glossary.filter((item) => ((inputtedTerm === "" && item.term.toLowerCase() !== inputtedTerm.toLowerCase()) || (inputtedTerm !== "" && item.term.toLowerCase() === inputtedTerm.toLowerCase())));
  filterArray = filterArray.filter((item) => ((selectedClass === 99 && item.class !== selectedClass) || (selectedClass !== 99 && item.class === selectedClass)));
  if (checkedTag.length !== 0) {
    filterArray.forEach(obj => {
      if (checkedTag.some(item => obj.tags.includes(item))) {
        result.push({ term: obj.term, class: obj.class, definition: obj.definition, tags: obj.tags });
      };
    });
  }
  else {
  ...
  let p = document.createElement("p");
  p.innerHTML = `<b>Total Search Result : ${result.length}</b>`;
  div.appendChild(p);
...
txtTerm.addEventListener('keyup', (e) => { 
  let inputtedTerm = e.target.value;
  let selectedClass = document.getElementById("searchClass").value;
  let checkedTag = document.querySelectorAll("input[type='checkbox']:checked");
  //searchTerm(inputtedTerm);
  searchAll(inputtedTerm, selectedClass, checkedTag)
});

ddlClass.addEventListener('change', (e) => {
  let inputtedTerm = document.getElementById("searchTerm").value;
  let selectedClass = parseInt(e.target.value);
  let checkedTag = document.querySelectorAll("input[type='checkbox']:checked");
  //searchClass(selectedClass);
  searchAll(inputtedTerm, selectedClass, checkedTag)
});

cbTag.forEach(function(chk){
  chk.addEventListener('click', function() {
    let inputtedTerm = document.getElementById("searchTerm").value;
    let selectedClass = parseInt(document.getElementById("searchClass").value);
    if(this.checked) {
      checkedTagList.push(this.value);
    }
    else {
      checkedTagList = checkedTagList.filter(value => value !== this.value)
    }
    //searchCheckbox(checkedTagList);
    searchAll(inputtedTerm, selectedClass, checkedTagList)
  });
});
